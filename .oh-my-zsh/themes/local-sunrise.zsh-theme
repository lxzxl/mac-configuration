...
function custom_git_prompt() {
  ...
  echo "$ZSH_THEME_GIT_PROMPT_PREFIX%B${ref#refs/heads/}$(parse_git_dirty)$(git_prompt_ahead)$(custom_git_prompt_status)$ZSH_THEME_GIT_PROMPT_SUFFIX"
}

# %B sets bold text
PROMPT='$(ssh_connection)$(prompt_user)  %{$M%}%{$C%}%B%2~ $(custom_git_prompt)%{$M%}%B»%b%{$RESET%} '
RPS1="${return_code}"
